...
async def list_relationships(
    ...
    rows = await q(
        ...
        f"coalesce(a.id, a.name, a.address) AS from_id, "
        f"coalesce(b.id, b.name, b.address) AS to_id "
        f"LIMIT $limit",
        limit=limit
    )
    return rows
...
async def create_relationship(body: RelCreate):
    ...
    rows = await q(
        ...
        f"WHERE (a.id = $from_id OR a.name = $from_id OR a.address = $from_id) "
        f"AND   (b.id = $to_id   OR b.name = $to_id   OR b.address = $to_id) "
        f"CREATE (a)-[r:{body.rel_type} {{{prop_str}}}]->(b) "
        f"RETURN id(r) AS rel_id, properties(r) AS props",
        from_id=body.from_id, to_id=body.to_id, **props
    )
    if not rows: raise HTTPException(400, "Nodes not found")
    return rows[0]

# ── ADD / UPDATE propiedad en 1 relación ─────────────────────────────────
...
async def set_rel_property(rel_type: str, rel_id: int, body: RelPropOp):
    ...
    rows = await q(
        ...
        f"SET r[$prop] = $value RETURN id(r) AS rel_id, properties(r) AS props",
        rid=rel_id, prop=body.prop, value=body.value
    )
    if not rows: raise HTTPException(404)
    return rows[0]

# ── DELETE propiedad en 1 relación ───────────────────────────────────────
...
async def remove_rel_property(rel_type: str, rel_id: int, prop_name: str):
    ...
    rows = await q(
        f"MATCH ()-[r:{rel_type}]->() WHERE id(r) = $rid "
        f"REMOVE r[$prop] RETURN id(r) AS rel_id, properties(r) AS props",
        rid=rel_id, prop=prop_name
    )
    if not rows: raise HTTPException(404)
    return rows[0]

# ── UPDATE propiedad en MÚLTIPLES relaciones ─────────────────────────────
...
async def delete_relationship(rel_type: str, rel_id: int):
    if rel_type not in ALLOWED_RELS:
        raise HTTPException(400, "Relationship type not allowed")
    await q(f"MATCH ()-[r:{rel_type}]->() WHERE id(r) = $rid DELETE r", rid=rel_id)
