"""
Carga todos los CSVs a Neo4j Desktop.
1. Copia los archivos CSV a la carpeta import/ de Neo4j.
2. Crea constraints e índices.
3. Carga nodos y relaciones via LOAD CSV.
...
# ── Copiar CSVs al import folder ───────────────────────────────────────────────
def copy_csvs():
    os.makedirs(NEO4J_IMPORT, exist_ok=True)
    files = [f for f in os.listdir(CSV_DIR) if f.endswith('.csv')]
    print(f"Copiando {len(files)} archivos CSV a Neo4j import/...")
    for f in files:
        shutil.copy2(os.path.join(CSV_DIR, f), os.path.join(NEO4J_IMPORT, f))
    print("  ✓ Copia completada\n")

# ── Runner ─────────────────────────────────────────────────────────────────────
def run(session, label, query, params=None):
    start = time.time()
    ...
    props_c = summary.counters.properties_set
    print(f"  ✓ {label:<45} "
          f"nodos={nodes_c:>5}  rels={rels_c:>5}  props={props_c:>6}  "
          f"({elapsed:.1f}s)")

# ── Constraints e índices ──────────────────────────────────────────────────────
CONSTRAINTS = [
    ("constraint board_id",    "CREATE CONSTRAINT board_id    IF NOT EXISTS FOR (n:Board)    REQUIRE n.id IS UNIQUE"),
    ("constraint thread_id",   "CREATE CONSTRAINT thread_id   IF NOT EXISTS FOR (n:Thread)   REQUIRE n.id IS UNIQUE"),
    ("constraint post_id",     "CREATE CONSTRAINT post_id     IF NOT EXISTS FOR (n:Post)     REQUIRE n.id IS UNIQUE"),
    ("constraint user_id",     "CREATE CONSTRAINT user_id     IF NOT EXISTS FOR (n:User)     REQUIRE n.id IS UNIQUE"),
    ("constraint tag_name",    "CREATE CONSTRAINT tag_name    IF NOT EXISTS FOR (n:Tag)      REQUIRE n.name IS UNIQUE"),
    ("constraint file_id",     "CREATE CONSTRAINT file_id     IF NOT EXISTS FOR (n:File)     REQUIRE n.id IS UNIQUE"),
    ("constraint report_id",   "CREATE CONSTRAINT report_id   IF NOT EXISTS FOR (n:Report)   REQUIRE n.id IS UNIQUE"),
    ("constraint ban_id",      "CREATE CONSTRAINT ban_id      IF NOT EXISTS FOR (n:Ban)      REQUIRE n.id IS UNIQUE"),
    ("constraint reaction_id", "CREATE CONSTRAINT reaction_id IF NOT EXISTS FOR (n:Reaction) REQUIRE n.id IS UNIQUE"),
    ("constraint ip_address",  "CREATE CONSTRAINT ip_address  IF NOT EXISTS FOR (n:IP)       REQUIRE n.address IS UNIQUE"),
    ("index post_created",     "CREATE INDEX post_created  IF NOT EXISTS FOR (n:Post)   ON (n.created_at)"),
    ("index post_score",       "CREATE INDEX post_score    IF NOT EXISTS FOR (n:Post)   ON (n.score)"),
    ("index thread_created",   "CREATE INDEX thread_board  IF NOT EXISTS FOR (n:Thread) ON (n.created_at)"),
    ("index user_alias",       "CREATE INDEX user_alias    IF NOT EXISTS FOR (n:User)   ON (n.alias)"),
]
...
def main():
    ...
    with driver.session() as session:
        ...
        total_rels = result2.single()["total"]

        result3 = session.run("""
            MATCH (n) WHERE NOT (n)--() RETURN count(n) AS isolated
        """)
        isolated = result3.single()["isolated"]

        print(f"  Nodos totales   : {total}")
        print(f"  Relaciones      : {total_rels}")
        print(f"  Nodos aislados  : {isolated} {'✓ grafo conexo' if isolated == 0 else '✗ hay nodos sin relación'}")
        print(f"  Mínimo 5000     : {'✓' if total >= 5000 else '✗'} ({total} nodos)")
